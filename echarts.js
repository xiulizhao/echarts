(function (root, factory) {
  ...
  var loadDependence = function (fncallback) {
    ...
    if (!window.wwload.echarts) {
      window.wwload.echarts = "wait";
      // requirejs.config({
      //   paths: {
      //     "echarts": "插件路径" // 示例: libs/qrcodejs/qrcode
      //   }
      // });
      require(["libs/echarts/echarts.min"], function (echarts) {
        window.wwload.echarts = true;
        window.echarts = echarts;
        replace();
        fncallback();
      });
    } else if (window.wwload.echarts === "wait") {
      setTimeout(function () {
   ...
   */
  function echarts_btn($ele) {
    var config = {};
    config.echartsid = $ele.attr("data-echartsid");
    var echartsoption = $ele.attr("data--echartsoption");
    //console.log(echartsoption);
    $ele.find(".container_btn").attr("id", config.echartsid);

    var echartsid = "#" + config.echartsid;

    var echartsdom = $ele.find(echartsid)[0];
    var echartsdata = echartsoption.replace(/[\r]/g, "\\r").replace(/[\n]/g, "\\n")
    var myChart = echarts.init(echartsdom);

    myChart.on('click', function (params) {
      //console.log(params);
      //console.log(params.name);
      $.wwclass.helper.updateProp($ele, "data-x-paramsname", params.name);   //点击获取当前数据名称
      //console.log(params.value);
      $.wwclass.helper.updateProp($ele, "data-x-paramsvalue", params.value);   //点击获取当前数据值
      //console.log(params.percent);
      $.wwclass.helper.updateProp($ele, "data-x-paramspercent", params.percent);  //点击获取当前数据百分比值

    });

    var option = eval('(' + echartsdata + ')');
    //console.log(option);
  ...
  function processElement($ele) {
    ...
    // setTimeout(function () {
    //   $.wwclass.helper.updateProp($ele, "data-x-inited", true);
    //   $ele.find(".container_btn").removeClass("hidden");
    // }, 100);
    setTimeout(function () {
      echarts_btn($ele);
    }, 1000)
